...
class UserService:
    """
    Generic user service that works with **any** ``BaseDocument`` subclass.
    ...
    def user_register(self, data: dict = None):
        try:
            hashed_password = get_password_hash(data["password"])
            user = self.model.find_one({"username": data["username"]})
    ...
    def user_delete(self, username: str = None):
        try:
            user = self.model.find_one({"username": username})
            if not user["status"]:
                ...
                return status
            result = self.model.delete_one({"username": username})
